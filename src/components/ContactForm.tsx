...
export const ContactForm = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!contactValue) {
            ...
            const isTelegram = /^@[a-zA-Z0-9_]{4,}$/.test(contactValue) || /^(https?:\/\/)?(t\.me|telegram\.me)\/[a-zA-Z0-9_]{4,}$/.test(contactValue);

            if (!isEmail && !isTelegram) {
                newErrors.contact = "Введите корректный email или Telegram (напр. @username)";
            }
        }
                                ...
                                >
                                    ...
                                    <div className="relative">
                                        <div className="space-y-2">
                                            <label className={`text-xs font-black uppercase tracking-widest block transition-colors ${errors.name ? 'text-red-500' : ''}`}>Как вас зовут?</label>
                                            <input
                                                type="text"
                                                value={formData.name}
                                                onChange={(e) => handleChange('name', e.target.value)}
                                                placeholder="Иван Иванов"
                                                className={`w-full bg-transparent border-b-2 py-2 focus:outline-none font-bold placeholder:text-gray-300 transition-colors ${errors.name ? "border-red-500 text-red-500 focus:border-red-500" : "border-black focus:border-gray-500"
                                                    }`}
                                            />
                                        </div>
                                        <AnimatePresence>
                                    ...
                                    <div className="relative pt-2">
                                        <div className="space-y-2">
                                            <label className={`text-xs font-black uppercase tracking-widest block transition-colors ${errors.contact ? 'text-red-500' : ''}`}>Ваш Email / Telegram</label>
                                            <input
                                                type="text"
                                                value={formData.contact}
                                                onChange={(e) => handleChange('contact', e.target.value)}
                                                placeholder="@username or email"
                                                className={`w-full bg-transparent border-b-2 py-2 focus:outline-none font-bold placeholder:text-gray-300 transition-colors ${errors.contact ? "border-red-500 text-red-500 focus:border-red-500" : "border-black focus:border-gray-500"
                                                    }`}
                                            />
                                        </div>
                                        <AnimatePresence>
                                    ...
                                    <div className="relative pt-2">
                                        <div className="space-y-2">
                                            <label className={`text-xs font-black uppercase tracking-widest block transition-colors ${errors.message ? 'text-red-500' : ''}`}>О чем ваш проект?</label>
                                            <textarea
                                                rows={4}
                                                value={formData.message}
                                                onChange={(e) => handleChange('message', e.target.value)}
                                                placeholder="Расскажите немного о ваших целях..."
                                                className={`w-full bg-transparent border-b-2 py-2 focus:outline-none font-bold placeholder:text-gray-300 resize-none transition-colors ${errors.message ? "border-red-500 text-red-500 focus:border-red-500" : "border-black focus:border-gray-500"
                                                    }`}
                                            ></textarea>
                                        </div>
                                        <AnimatePresence>
                                    ...
                                    <button type="submit" className="brutal-btn w-full flex items-center justify-center gap-3 py-4 text-xl mt-8">
